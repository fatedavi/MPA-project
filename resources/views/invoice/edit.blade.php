<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('invoice.update', $invoice->id) }}" class="space-y-8" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="bg-green-50 border border-green-200 rounded-lg p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="client_id" id="client_id" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" onchange="fillClientData()">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="up" id="up" value="{{ $invoice->up }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="UP will be auto-filled from client name" readonly>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea name="alamat_client" id="alamat_client" rows="2" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="Client address will be auto-filled" readonly>{{ $invoice->alamat_client }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
                            ...
                            <div id="invoice-items" class="space-y-4">
                                @if($invoice->detail_invoice && count($invoice->detail_invoice) > 0)
                                    ...
                                    <div class="invoice-item bg-white border border-yellow-300 rounded-lg p-4 shadow-sm {{ $index === 0 ? 'bg-yellow-50' : '' }}">
                                        <div class="space-y-4">
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Description *</label>
                                                <input type="text" name="detail_invoice[{{ $index }}][deskripsi]" value="{{ $item['deskripsi'] }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="Item description">
                                            </div>
                                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700">Qty *</label>
                                                    <input type="number" name="detail_invoice[{{ $index }}][qty]" min="0.01" step="0.01" value="{{ $item['qty'] }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="1" onchange="calculateItemTotal(this)">
                                                </div>
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700">Satuan</label>
                                                    <input type="text" name="detail_invoice[{{ $index }}][satuan]" value="{{ $item['satuan'] ?? 'pcs' }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="pcs, jam, hari">
                                                </div>
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700">Harga *</label>
                                                    <input type="number" name="detail_invoice[{{ $index }}][harga]" min="0" step="0.01" value="{{ $item['harga'] }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="0" onchange="calculateItemTotal(this)" onblur="formatHarga(this)">
                                                    <p class="mt-1 text-xs text-gray-500">Contoh: 5321.50</p>
                                                </div>
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700">Total</label>
                                                    <input type="text" name="detail_invoice[{{ $index }}][total]" value="{{ 'Rp ' . number_format($item['total'], 2, ',', '.') }}" readonly class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-100 sm:text-sm" placeholder="Rp 0">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                @else
                                    ...
                                    <div class="invoice-item bg-white border border-yellow-300 rounded-lg p-4 shadow-sm bg-yellow-50">
                                        <div class="space-y-4">
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700">Description *</label>
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                                ...
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700">Satuan</label>
                                                    <input type="text" name="detail_invoice[0][satuan]" value="pcs" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="pcs, jam, hari">
                                                </div>
                                                <div>
                                                    <label class="block text-sm font-medium text-gray-700">Harga *</label>
                        ...
                        <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="total_invoice" id="total_invoice" value="{{ 'Rp ' . number_format($invoice->total_invoice, 2, ',', '.') }}" readonly class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-100 sm:text-sm" placeholder="Rp 0">
                                ...
                                <div>
                                    ...
                                    <select name="nama_bank" id="nama_bank" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" onchange="fillBankData()">
                                        ...
                                        @foreach($banks as $bank)
                                            <option value="{{ $bank->nama_bank }}" data-an="{{ $bank->an }}" data-ac="{{ $bank->ac }}" {{ $bank->nama_bank === $invoice->nama_bank ? 'selected' : '' }}>{{ $bank->nama_bank }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="an" id="an" value="{{ $invoice->an }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="Account name will be auto-filled" readonly>
                                </div>
                                
                                <div>
                                    <label for="ac" class="block text-sm font-medium text-gray-700">Account Number *</label>
                                    <input type="text" name="ac" id="ac" value="{{ $invoice->ac }}" required class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-[#8D0907] focus:border-[#8D0907] sm:text-sm" placeholder="Account number will be auto-filled" readonly>
                                </div>
    ...
    <script>
        let itemCount = {{ $invoice->detail_invoice ? count($invoice->detail_invoice) : 1 }};
